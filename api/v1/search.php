...
if ($method == 'POST') {
  ...
  $req = $db->prepare('SELECT * FROM product WHERE (name LIKE ? OR description LIKE ? OR SKU = ? OR color LIKE ? OR size = ?) AND category_id IN (SELECT id FROM product_category WHERE name LIKE ?);');
  $req->execute(array(
    '%'.$data['search'].'%',
    '%'.$data['search'].'%',
    $data['search'],
